...
from train import train_reconstruct_property_edge_direction as train_epoch
from train import test_reconstruct_property_edge_direction as test_epoch
...
while epoch<=args.epochs:
    ...
    if epoch % args.epochs_log == 0:
        with open('reconstruct_property_edge_direction_training_losses.txt', 'w') as f:
            f.write(str(rolling_loss))
        print('Epoch: {}/{}, train loss: {:.4f} in {:.2f} secs'.format(
            epoch, args.epochs, np.mean(rolling_loss[-100:]), t))
        l = test_epoch(epoch, args, gcn, fc, (val_adj, val_feat))
        print('Epoch: {}/{}, validation loss: {:.4f}'.format(
            epoch, args.epochs, l))
        with open('reconstruct_property_edge_direction_test_losses.txt', 'a+') as f:
            f.write(str(l) + '\n')
    epoch += 1

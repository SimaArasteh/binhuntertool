...
def train(data, n_way, num_epochs=1000, num_shots=1):
    ...
    with tf.Session( graph = graph ) as sess:
        ...
        for epoch in range(num_epochs):
            ...
            while True:
                ...
                for i, batch in enumerate(construct_batch(ft_test_adj, ft_test_features, ft_test_labels, batch_size)):
                    ...
                    batch_loss.append(outs[0])
                    batch_acc.append(outs[1])

                loss.append(round(np.mean(batch_loss), 2))
                acc.append(np.mean(batch_acc))
                
                print("Epoch:", '%04d' % (i + 1),
                  "val_loss=", "{:.2f}".format(loss[-1]),
                  "val_acc=", "{:.2f}".format(acc[-1]), 
                  "time=", "{:.2f}".format(time.time() - t))

                i += 1
                if len(loss) > stopping_parameter:
                    decreasing_loss = [True for j in range(stopping_parameter, 1, -1) if loss[-j] > loss[-(j-1)]]
        ...
        print("total average loss = ", "total average val loss = ", "{:.2f}".format(np.mean(avg_loss)),
              "total average val acc = ", "{:.2f}".format(np.mean(avg_acc)))
...
if __name__ == '__main__':
    ...
    train(data, num_shots=num_shots, num_epochs=num_epochs, n_way=n_way)
